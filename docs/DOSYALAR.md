...
| `swap.rs` | Takas alani: bolumun sonundaki yuvalar, yuva bitmap'i, cerceve <-> disk aktarimi ve sayaclar. |
...
| Olum sebebi | `scheduler.rs` (`Task.exit_signal`), `exceptions.rs` (`signal_of_vector`), `nt_syscalls.rs` (`win32_exit_code`) |
